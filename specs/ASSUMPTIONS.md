...
- **Client push mechanism**: the system uses **WebSocket** as the primary realtime channel for both active players and spectators (see resolved decision R12). WebSocket provides full-duplex bidirectional communication over a single persistent connection, enabling clients to submit commands and receive state updates on the same channel with sub-second latency. A long-poll fallback is acceptable for hostile network environments but WebSocket is the default.
